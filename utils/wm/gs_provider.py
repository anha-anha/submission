    ...
    def __recover_messages_from_latents(self,
        ...
        for i in range(self.offset,
            ...
            for zsT_pixel_value in np.nditer(latent):
                ...
                y_reconstructed = norm.cdf(zsT_pixel_value) * 2**self.l  # in rare cases, this can be a 2 for l=1. Should never happen
            ...
            for n in encrypted_numbers:
                ...
                encrypted_bits.append(bin(n)[2:].zfill(self.l))  # TODO: this can, in some cases 
            ...
            )  # replicated_message_bytes = m_reconstructed_bytes
